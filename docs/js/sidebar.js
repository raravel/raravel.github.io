$(function () {
  function closeSidebar() {
    ...
    $('#sidebar-background').fadeOut(200);
  ...
  function sidebarCallback() {
    if (/*!isMobile() &&*/ $('#sidebar').is(':visible')) {
      var isOpen = $('#toggle-sidebar').hasClass('on')
      isOpen ? $('#toggle-sidebar').removeClass('on') : $('#toggle-sidebar').addClass('on')
      if (isOpen) {
        closeSidebar();
      } else {
        openSidebar();
  ...
  $('#sidebar-background').on('click', sidebarCallback)
  $('#toggle-sidebar').on('click', sidebarCallback)
  $('#toggle-sidebar-hbg').on('click', sidebarCallback)
